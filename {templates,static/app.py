...
def rota_por_perfil(perfil):
    """Retorna a rota inicial correta para cada perfil."""
    if perfil == "usuario":
        return url_for("sos_page")
    return url_for("dashboard")

@app.route("/")
def index():
    if "user_id" in session:
        return redirect(rota_por_perfil(session.get("perfil","usuario")))
    return redirect(url_for("login"))
...
def login():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            return redirect(rota_por_perfil(user["perfil"]))
...
@login_required
def dashboard():
    # Perfil usuário só acessa o SOS
